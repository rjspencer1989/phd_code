...
class TestUndoRollback(unittest.TestCase):
    ...
    def setUp(self):
        ...
        notif_res = self.notification_doc = self.db.get(self.notification_doc['_id'])
        dt = datetime.datetime(2015, 2, 12, hour=14, minute=34)
        self.hist4 = self.add_history_item(self.title, self.description, notif_res['id'], notif_res['rev'], 'notifications', 'add', True, ts=dt.isoformat())
