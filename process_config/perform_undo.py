...
class UndoProcessor(threading.Thread):
    ...
    def perform_undo(self, current_doc):
        ...
        if undo_doc['collection'] == 'devices':
            undone_rev = self.undo_device_change(undo_doc, current_doc['doc_rev'])
        else:
        ...
        if undone_rev != '':
            ...
            add_history.add_history_item("Undo Configuration change", "Undo of %s" % (current_doc['description']), current_doc['doc_id'], undone_rev, True)
