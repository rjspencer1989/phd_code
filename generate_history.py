...
def generate():
    ...
    connection_state_instance = connection_state.ConnectionState()
    res = db.save_doc(connection_state_instance.get_doc(), force_update=True)
    random.seed(1)
    ...
    wifi_instance = wifi.Wifi(ssid, password, 'n', channel, timestamp=dt.isoformat())
    res = db.save_doc(wifi_instance.get_doc(), force_update=True)
    ...
    dt = datetime(2014, 01, 12, hour=10, minute=31, tzinfo=tzutc())
    ...
    dt = datetime(2014, 01, 12, hour=11, minute=00, tzinfo=tzutc())
    ...
    dt = datetime(2014, 01, 12, hour=19, minute=35, tzinfo=tzutc())
    ...
    dt = datetime(2014, 07, 25, hour=18, minute=25, tzinfo=tzutc())
    ...
    dt = datetime(2014, 12, 27, hour=13, minute=0, tzinfo=tzutc())
    mrltablet6.set_field("event_timestamp", dt.isoformat())
    res = db.save_doc(mrltablet6.get_doc(), force_update=True)

    dt = datetime(2014, 01, 12, hour=10, minute=32, tzinfo=tzutc())
